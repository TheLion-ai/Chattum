...
def test_put_source_pdf(test_client, api_key) -> None:
...
def test_put_source_url(test_client, api_key) -> None:
    """Test the put source endpoint."""
...
def test_get_sources(test_client, api_key) -> None:
    """Test the get sources endpoint."""
    response = test_client.get(
        f"/{username}/bots/{bot_id}/sources", headers={"X-API-Key": api_key}
    )
    assert response.status_code == 200
...
def test_get_source_pdf(test_client, api_key) -> None:
    """Test the get source endpoint."""
    response = test_client.get(
        f"/{username}/bots/{bot_id}/sources/{source_id_pdf}",
        headers={"X-API-Key": api_key},
    )
    assert response.status_code == 200
...
def test_get_sources_not_found(
    test_client, api_key, id="123456789012345678901234"
) -> None:
    """Test the get sources endpoint with a bot that does not exist."""
    fake_id = ObjectId(id)
    response = test_client.get(
        f"/{username}/bots/{fake_id}/sources", headers={"X-API-Key": api_key}
    )
    assert response.status_code == 404
    assert response.json() == {"detail": "Bot not found"}


def test_delete_source_pdf(test_client, api_key) -> None:
    ...
    assert response.status_code == 200
    test_get_sources_not_found(test_client, api_key, source_id_pdf)


def test_delete_source_not_found(test_client, api_key) -> None:
    """Test the delete source endpoint with a bot that does not exist."""
    fake_id = ObjectId("123456789012345678901234")
    response = test_client.delete(
        f"/{username}/bots/{fake_id}/sources/{source_id_pdf}",
        headers={"X-API-Key": api_key},
    )
    assert response.status_code == 404


def test_delete_source_url(test_client, api_key) -> None:
    """Test the delete source endpoint."""
    response = test_client.delete(
        f"/{username}/bots/{bot_id}/sources/{source_id_url}",
        headers={"X-API-Key": api_key},
    )
    assert response.status_code == 200
    test_get_sources_not_found(test_client, api_key, source_id_url)
