...
@router.get(
    "",
    response_model=pm.PromptResponse,
)
def get_prompt(bot_id: str, username: str, auth=Depends(check_key)) -> str:
...
@router.put(
    "",
    response_model=pm.MessageResponse,
)
def change_prompt(
    bot_id: str, username: str, request: pm.PromptRequest, auth=Depends(check_key)
) -> pm.MessageResponse:
