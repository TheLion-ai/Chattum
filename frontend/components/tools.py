...
from backend_controller import get_available_tools, create_new_tool, get_tools, delete_tool
...
class ToolsPanel:
    ...
    def __init__(self,bot_id):
        ...
        self.available_tools_dict = {tool["name"]: tool for tool in self.available_tools}
    ...
    def __call__(self):
        ...
        with tools_list:
            for tool in self.tools:
                st.button(tool["name"],use_container_width=True, on_click = query_params.set_to_url , kwargs = {"tool_id":tool["id"]}, type="primary" if self._selected_tool == tool else "secondary", key = f"tool_{tool['id']}")
        with tool_content:
            if self._selected_tool is not None:
                self._display_tool_form(self._selected_tool, key = f"{self._selected_tool['id']}_form")
        ...
        with st.form(key=key):
            for user_variable in tool_variables:
                ...
                if user_variable["form_type"] == "text":
                    ...
                    user_variable["value"] = st_ace(language="handlebars", value = user_variable.get("value", ""))
            col1, _, col2 = st.columns([2,3,2])
            with col1:
                submit_button = st.form_submit_button("Update tool", type="primary",use_container_width=True)
                if submit_button:
                    create_new_tool(self.bot_id, tool["name"], tool_variables)
                    st.rerun()
            with col2:
                delete_button = st.form_submit_button("🗑️Delete tool", type="secondary", use_container_width=True)
                if delete_button:
